...
class CMAES:
    ...
    def __init__(self, num_params,      # number of model parameters
                 ...
                 weight_decay=0.01,
                 mean=0):    # weight decay coefficient
        ...
        if type(mean) == float:
            print("mean is float")
            self.es = cma.CMAEvolutionStrategy(self.num_params * [mean],
                                           self.sigma_init,
                                           {'popsize': self.popsize,
                                            })
        else:
            self.es = cma.CMAEvolutionStrategy(mean,
                                           self.sigma_init,
                                           {'popsize': self.popsize,
                                            })
...
class PEPG:
    ...
    def __init__(self, num_params, # number of model parameters
        ...
        if type(mean) == float:
            print("mean is float")
            self.mean= np.array(num_params * [mean])
        else:
            self.mean = mean
    ...
    def reset_popsize(self, popsize):
        self.popsize = popsize
        if self.average_baseline:
            assert (self.popsize % 2 == 0), "Population size must be even"
            self.batch_size = int(self.popsize / 2)
...
class SPSA:
    ...
    def __init__(self,
        ...
        if weights is None:
            weights = np.random.randn(num_params)/num_params 
    ...
    def ask(self):
        ...
        self.delta_vec = np.random.randint(0,2,size=(self.num_params))
        self.delta_vec[self.delta_vec == 0] = -1
        self.var_vec = np.var(self.delta_vec)
        
        self.params = np.zeros(shape=(2,self.num_params))
        
        self.params[0,:] = self.weights + self.epsilon * self.delta_vec
        self.params[1,:] = self.weights - self.epsilon * self.delta_vec
    ...
    def tell(self, error):
        ...
        grad = ((e_plus - e_minus)/(2*self.epsilon *self. var_vec) ) * self.delta_vec
